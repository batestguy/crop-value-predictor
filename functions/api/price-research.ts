...
type RequestBody = { cropId?: unknown; state?: unknown; priceType?: unknown }
...
function parseAnswer(answer: unknown, cropId: string, state: string, priceType: 'retail' | 'wholesale', results: unknown[]) {
  ...
  return {
    ...
    sources, warnings: ['No usable local observation was found; this is a web-grounded estimate and requires farmer confirmation.', ...(divisor === 1000 ? ['The provider returned a metric-ton value; it was converted to NGN/kg by dividing by 1,000.'] : [])],
  }
}
...
export async function onRequest(context: { request: Request; env: Env }) {
  ...
  if (!SEARCH_TERMS[cropId] || !NIGERIAN_STATES.has(state.toLowerCase()) || !priceType) return json({ error: 'Choose a supported crop, Nigerian state, and price type.' }, 400)

  const controller = new AbortController()
  const timeout = setTimeout(() => controller.abort(), 30_000)
  try {
    const query = `${state} ${SEARCH_TERMS[cropId]} market price Nigeria 2026 ${priceType} per kilogram`
    ...
    const result = parseAnswer(payload.answer, cropId, state, priceType, Array.isArray(payload.results) ? payload.results : [])
